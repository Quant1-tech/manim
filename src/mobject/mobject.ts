/** @file Base classes for objects that can be displayed. */
...
import {$add, $apply, $scale, $sub, Pt3, Vec3} from "../utils/js";
import {rotationMatrix} from "../utils/space_ops";
...
import {listUpdate} from "../utils/iterables";

/**
 * Mathematical Object: base class for objects that can be displayed on screen.
 */
export class Mobject {
  /** Local transformation matrix for $rendering */
  $matrix: Matrix4;
  ...
  constructor({
    ...
    this.color = color;
    // this.name = this._Class__._Name__ if name is None else name
    this.dim = dim;
    // this.target = target
    // this.zIndex = zIndex
    // this.pointHash = None
  ...
  getFamily(recurse = true) {
    ...
    // subFamilies = list(map(Mobject.getFamily, this.submobjects))
    // allMobjects = [this] + list(it.chain(*subFamilies))
  ...
  //             ``.animate``
  //              will interpolate the {@link Mobject} between its points prior to
  ...
  add(...mobjects: Mobject[]): this {
    ...
    this.submobjects = listUpdate(this.submobjects, mobjects);
    return this;
  }
  ...
  /**
   * Create and return an identical copy of the {@link Mobject} including all {@link submobjects}.
   * @returns The copy.
   */
  copy(): Mobject {
  ...
  //                     self.play(Rotating(dot, about_point=ORIGIN, angle=TWOPI, run_time=TWOPI, rate_func=linear))
  ...
  shift(...vectors: Pt3[]): this {
    ...
    this.$matrix.multiply(new Matrix4().makeTranslation(...totalVector));
    return this;
  }
   ...
   * @param scaleFactor The scaling factor `\alpha`. If `0 < |\alpha| < 1`, the mobject will shrink,
   *                    and for `|\alpha| > 1` it will grow. Furthermore, if `\alpha < 0`, the mobject is also flipped.
   */
  scale(scaleFactor: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
    this.applyPointsFunctionAboutPoint(pt => pt.map(xi => xi * scaleFactor) as Pt3, kwargs);
    this.$matrix.multiply(new Matrix4().makeScale(scaleFactor, scaleFactor, scaleFactor));
    return this;
  }
   ...
   */
  rotateAboutOrigin(angle: number, axis = OUT): this {
   ...
   */
  rotate(angle: number, {axis = OUT, aboutPoint = ORIGIN, ...kwargs}: {axis?: Vec3} & Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
    const rotMatrix = rotationMatrix(angle, axis);
    this.applyPointsFunctionAboutPoint(pt => $apply(rotMatrix, pt), {aboutPoint, ...kwargs});
    this.$matrix.multiply(new Matrix4().makeRotationAxis(new Vector3(...axis), angle));
    return this;
  }
  ...
  //         """
  //         return self.rotate(TWOPI / 2, axis, **kwargs)

  stretch(factor: number, dim: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
    const func = (pt: Pt3) => {
  ...
  //                     self.play(t.animate.set_value(TWOPI), run_time=3)
  //         """
  ...
  //         self.rotate(TWOPI / 14, RIGHT + UP, **kwargs)
  //         return self
  ...
  rescaleToFit(length: number, dim: number, stretch = false, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  scaleToFitWidth(width: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  stretchToFitWidth(width: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  scaleToFitHeight(height: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  stretchToFitHeight(height: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  scaleToFitDepth(depth: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
  ...
  stretchToFitDepth(depth: number, kwargs: Parameters<this["applyPointsFunctionAboutPoint"]>[1] = {}): this {
    return this.rescaleToFit(depth, 2, true, kwargs)
  }

  setCoord(value: number, dim: number, direction = ORIGIN): this {
  ...
  moveTo(pointOrMobject: Pt3 | Mobject, {alignedEdge = ORIGIN, coorMask = [1, 1, 1] as Pt3} = {}): this {
    let target: Pt3;
   ...
   * of color 
   */
  setColor(color = YELLOW_C, family: boolean = true): this {
    if (family) {
  ...
  getMergedArray(arrayAttr: string): Pt3[] {
   ...
   */
  getCriticalPoint(direction: number[]): Pt3 {
  ...
  //         """Get coordinates of the middle of the path that forms the  {@link `.}
  //         Examples
  //         --------
  ...
  /** Match the color with the color of another {@link Mobject} */
  matchColor(mobject: Mobject): this {
    return this.setColor(mobject.getColor());
  }

  /** Match the specified dimension with the dimension of another {@link Mobject} */
  matchDimSize(mobject: Mobject, dim: number): this {
    return this.rescaleToFit(mobject.lengthOverDim(dim), dim);
  }

  /** Match the width with the width of another {@link Mobject} */
  matchWidth(mobject: Mobject): this {
    return this.matchDimSize(mobject, 0);
  }

  /** Match the height with the height of another {@link Mobject} */
  matchHeight(mobject: Mobject): this {
    return this.matchDimSize(mobject, 1);
  }

  /** Match the depth with the depth of another {@link Mobject} */
  matchDepth(mobject: Mobject): this {
    return this.matchDimSize(mobject, 2);
  }

  /** Match the coordinates with the coordinates of another {@link Mobject} */
  matchCoord(mobject: Mobject, dim: number, direction = ORIGIN): this {
    return this.setCoord(mobject.getCoord(dim, direction), dim, direction);
  }

  /** Match x coord. to the x coord. of another {@link Mobject} */
  matchX(mobject: Mobject, direction = ORIGIN): this {
    return this.matchCoord(mobject, 0, direction);
  }

  /** Match y coord. to the x coord. of another {@link Mobject} */
  matchY(mobject: Mobject, direction = ORIGIN): this {
    return this.matchCoord(mobject, 1, direction);
  }

  /** Match z coord. to the x coord. of another {@link Mobject} */
  matchZ(mobject: Mobject, direction = ORIGIN): this {
  ...
  //         alignment_vect=UP,
  //     ) {
  //         """Aligns mobject to another {@link Mobject} in a certain direction.
  ...
  //         **kwargs,
  //     ) {
  //         """Sorts {@link Mobject} next to each other on screen.
  ...
  //         """The simplest {@link Mobject} to be transformed to or from self.
  ...
  //         """If a {@link Mobject} with points is being aligned to
  ...
  //         """Turns this {@link Mobject} into an interpolation between ``mobject1``
  //         and ``mobject2``.
  ...
  //         to another {@link Mobject}

  //         .. note::

  //             If both match_height and match_width are ``True`` then the transformed {@link Mobject}
  //             will match the height first and then the width

  //         Parameters
  //         ----------
  //         match_height
  //             If ``True``, then the transformed {@link Mobject} will match the height of the original
  //         match_width
  //             If ``True``, then the transformed {@link Mobject} will match the width of the original
  //         match_depth
  //             If ``True``, then the transformed {@link Mobject} will match the depth of the original
  //         match_center
  //             If ``True``, then the transformed {@link Mobject} will match the center of the original
  //         stretch
  //             If ``True``, then the transformed {@link Mobject} will stretch to fit the proportions of the original
  ...
  //         to another {@link `,}while keeping the style unchanged.
  ...
  throwErrorIfNoPoints(callerName: keyof this): void {
    if (this.hasNoPoints()) {
  ...
  //         family: bool = True,
  //     ) -> "VMobject":
  //         """Sets the {@link `'} :attr:`z_index` to the value specified in `z_index_value`.
  ...
  //         """Sets the {@link `'} z coordinate to the value of :attr:`z_index`.
...
/** Groups together multiple {@link Mobject}s */
export class Group extends Mobject {
  constructor(mobjects: Mobject[], kwargs: ConstructorParameters<typeof Mobject>[0]) {
    super(kwargs);
    this.add(...mobjects);
  }
}
