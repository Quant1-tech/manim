/** @file Mobjects that are lines or variations of them. */

import {$add, $dot, $normalize, $scale, $sub, Pt3, Vec3} from "../../utils/js";
import {angleOfVector, normalize} from "../../utils/space_ops";
import {TipableVMobject} from "./arc";
...
//         return super().put_start_and_end_on(start, end)
   ...
   * @param vector The vector to which the line is projected.
   */
   getProjection(vector: Vec3): Vec3 {
    const unit = this.getUnitVector();
    return $add(this.start, $scale(unit, $dot($sub(vector, this.start), unit)));
  }
  ...
  setAngle(angle: number, aboutPoint: Pt3 = this.getStart()): this {
    this.rotate(
      angle - this.getAngle(),
      {aboutPoint}
    );
  ...
  setLength(): this {
    return this.scale(length / this.getLength());
  }
}
